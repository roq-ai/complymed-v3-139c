...
import { getFranchises } from 'apiSdk/franchises';
...
function DashboardPage() {
  ...
  const franchiseStat = useSWR<PaginatedInterface<FranchiseInterface>>(
    () => `/franchises`,
    () => getFranchises({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Franchises',
      result: franchiseStat?.data?.totalCount,
      isLoading: franchiseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Dashboards',
